...
impl VideoSegment {
    ...
    pub async fn download(
        ...
        let response = self
            .video
            .download_client
            .download_header(&self.video.url, &headers)
            .await?;
...
impl SegmentedVideo {
    ...
    pub async fn download(&mut self, options: &Options) -> Result<(), Box<dyn std::error::Error>> {
        if !self.folder.exists() {
            match std::fs::create_dir(&self.folder) {
                Ok(_) => {}
                Err(err) => {
        ...
        let segments_downloaded = Mutex::new(0);
        ...
        let tasks = self
            ...
            .into_iter()
            .map(|segment| async {
                if let Err(err) = segment.download(&self.folder).await {
                    return Err(err);
                }
                ...
                *segments_downloaded += 1;
                ...
                println!(
                    "Downloaded {:width$} / {:width$} segments ({:5.2}%)\t ({})",
                    *segments_downloaded,
                    total_segments,
